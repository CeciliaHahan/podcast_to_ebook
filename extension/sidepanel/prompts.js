export const DEFAULT_PROMPTS = {
  wnSystem: "你是 transcript working-notes 生成器。你的任务是把 transcript 压缩成 summary + sections + excerpts 的 JSON，供后续 outline 使用。不得输出 schema 之外的内容。",
  wnUser: `任务：把 transcript 转成用于后续生成 booklet 的 working notes。
...
{{workingNotes}}`,
  ...
  draftSystem: "你是 booklet draft 生成器。你的任务是把 working notes 和 booklet outline 写成 title + sections(body) 的 JSON。不得输出 schema 之外的内容。",
  draftUser: `任务：把 booklet outline 写成一个可阅读的 booklet draft。
只输出一个 JSON 对象，不要 markdown，不要解释，不要额外文字。
